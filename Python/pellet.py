...
def _get_pellet_path(gfile, device = 'd3d', port = 'tang', dist_outside = 0.003,
    ...
    R_traj2, Z_traj2, phi_traj2 = np.array([3.1, 5.0]), np.array([3.0, -4.25]), np.array([0.0, 0.0])
...
def scan_density_profile(top_folder, device = None, runid_prefix = None,
                              den0 = np.around(np.arange(1.8, 4.1, 0.2), 1),
                              den1 = np.arange(100, 1501, 100),
                              pellet_executable_loc = None):
    """
    Use the same equilibrium, scan the pellet size and velocity
    'top_folder' should contain a g file, a prof file, and a nml file to copy as templates
    This will generate a new folder for each requested pellet radius and velocity
    ...
    # Make a new folder for each pellet size and run xpellet in each one
    print("nml_orig: ", nml_orig)
    # print(existing_nml)
    
    for dd in den0:
        # print(dd)
        den1 = np.multiply(dd,0.25)
        # print(den1)
        # Need to copy over files to each folder, otherwise it breaks
        new_folder = 'den0_' + str(dd)
        # print(new_folder)
        if runid_prefix is None:
            runid = '_pellet_test_'
        else:
        ...
        if profile_fileloc:
            print("Calling with pfile.")
            nml = pnml.PelletNml(equilib_fileloc, profile_fileloc, device = device,
                                runid = runid,
                                existing_nml_fileloc = existing_nml)
        else:
            print("Calling without pfile.")
            nml = pnml.PelletNml(equilib_fileloc, profile_fileloc=None, device = device,
                            runid = runid,
                            existing_nml_fileloc = existing_nml)
            
        print("nml new: ", nml)
        nml.write_nml('nml_pellet.dat', input_dict = {'den0': dd, 'den1' : den1})

    
        with open("output_den0_"+str(dd)+'.out',"w") as output:
            subprocess.run([pellet_executable_loc],stdout=output)
        rename_output_files_without_spaces()
        os.remove(equilib_fileloc)
        if profile_fileloc:

...
INTEGER, PRIVATE, SAVE, ALLOCATABLE :: &
  mp_f(:)                !number of pol data points on surfaces [-]
...
REAL(KIND=rspec), PRIVATE, SAVE, ALLOCATABLE :: &
  f_f(:),              & !R*B_t on radial grid [m*T]
  ffp_f(:),            & !F*dF/dpsi on radial grid [rad*T]
  psi_f(:),            & !pol magnetic flux on radial grid [Wb/rad]
  rhop_f(:),           & !radial grid uniform in poloidal flux [-]
  q_f(:)                 !safety factor on radial grid [-]

REAL(KIND=rspec), PRIVATE, SAVE, ALLOCATABLE :: &
  x_f(:),              & !x grid for psi(x,y) [m]
  y_f(:),              & !y grid for psi(x,y) [m]
  psixy_f(:,:),        & !pol magnetic flux on (x,y) grid [Wb/rad]
  cspln_f(:,:,:)         !spline coefficients for psi(x,y)
...
REAL(KIND=rspec), PRIVATE, SAVE, ALLOCATABLE :: &
  arclen_f(:,:),       & !distance along surface from start [m]
  bp_f(:,:),           & !pol magnetic field on surfaces [T]
  xp_f(:,:),           & !x nodes on surfaces [m]
  yp_f(:,:),           & !y nodes on surfaces [m]
  btot_f(:,:),         & !tot magnetic field on surfaces [T]
  ctheta_f(:,:),       & !Theta on surfaces [-]
  xndgradb_f(:,:),     & !n.grad(B) on surfaces [T/m]
  xnthinv_f(:,:)         !|Btot|/Bp on surfaces [-]
...
psi_f(1:nr_f)=psimag+(psilim-psimag)*(/ (i-1,i=1,nr_f) /)**2/(nr_f-1)**2
rhop_f(1:nr_f)=(psi_f(1:nr_f)-psi_f(1))/(psi_f(nr_f)-psi_f(1))

!F=R*Bt
CALL LINEAR1_INTERP(nx_f,rhop_x,f_x,nr_f,rhop_f, &
                    f_f,iflag,message)
...
!F*F'
CALL LINEAR1_INTERP(nx_f,rhop_x,ffp_x,nr_f,rhop_f, &
                    ffp_f,iflag,message)
...
!q
CALL LINEAR1_INTERP(nx_f,rhop_x,q_x,nr_f,rhop_f, &
                    q_f,iflag,message)
...
psixy_f(1:nx_f,1:ny_f)=psi_xy(1:nx_f,1:ny_f)
cspln_f(:,:,:)=0
...
IF(k_grid == 1) THEN
  ...
  rinterp(:)=rhop_f(:)
  dpsidrho(1:nr_f)=ABS(psi_f(nr_f))
...
DO j=2,nr_f
  ...
  ydum(1:mp_f(j))=btot_f(1:mp_f(j),j)**2/bp_f(1:mp_f(j),j)
  c1(1)=arclen_f(mp_f(j),j)
  CALL LINEAR1_INTEG(0,mp_f(j),arclen_f(:,j),ydum,1,c1, &
                     c2,iflag,message)
...
DO j=2,nr_f
  ...
  ydum(1:mp_f(j))=1/btot_f(1:mp_f(j),j)**2/bp_f(1:mp_f(j),j)
  c1(1)=arclen_f(mp_f(j),j)
  CALL LINEAR1_INTEG(0,mp_f(j),arclen_f(:,j),ydum,1,c1, &
                     c2,iflag,message)
...
f(:,:)=0
f(1,1)=bmag_f
f(1,2:nr_f)=f_f(2:nr_f)/xp_f(1,2:nr_f)
CALL SPLINE1_INTERP(k_vopt,nr_f,rinterp,f,nr_r,rho_r, &
                    value,iflag,message)
...
!-------------------------------------------------------------------------------
f(:,:)=0
f(1,2:nr_f)=(zmax_f(2:nr_f)-zmin_f(2:nr_f))/(xp_f(1,2:nr_f)-rin_f(2:nr_f))
f(1,1)=f(1,2)
CALL SPLINE1_INTERP(k_vopt,nr_f,rinterp,f,nr_r,rho_r, &
                    value,iflag,message)
...
DO j=2,nr_f
  ...
  ydum(1:mp_f(j))=bp_f(1:mp_f(j),j)*(xp_f(1:mp_f(j),j)/btot_f(1:mp_f(j),j))**2
  c1(1)=arclen_f(mp_f(j),j)
  CALL LINEAR1_INTEG(0,mp_f(j),arclen_f(:,j),ydum,1,c1, &
                     c2,iflag,message)
  ...
  f(1,j)=c2(1)/dlobp_f(j)/dpsidrho(j)**2
...
DO j=2,nr_f-1
  ...
  c1(1)=arclen_f(mp_f(j),j)
  CALL LINEAR1_INTEG(0,mp_f(j),arclen_f(:,j),xp_f(:,j),1,c1, &
                     c2,iflag,message)
  ...
  f(1,j)=ABS(c2(1)/dlobp_f(j)/dpsidrho(j))
...
DO j=2,nr_f-1
  ...
  ydum(1:mp_f(j))=xp_f(1:mp_f(j),j)*xp_f(1:mp_f(j),j)*bp_f(1:mp_f(j),j)
  c1(1)=arclen_f(mp_f(j),j)
  CALL LINEAR1_INTEG(0,mp_f(j),arclen_f(:,j),ydum,1,c1, &
                     c2,iflag,message)
...
DO j=2,nr_f
  ...
  c1(1)=arclen_f(mp_f(j),j)
  CALL LINEAR1_INTEG(0,mp_f(j),arclen_f(:,j),bp_f(:,j),1,c1, &
                     c2,iflag,message)
...
DO j=2,nr_f-1
  ...
  ydum(1:mp_f(j))=xp_f(1:mp_f(j),j)**2/bp_f(1:mp_f(j),j)
  c1(1)=arclen_f(mp_f(j),j)
  CALL LINEAR1_INTEG(0,mp_f(j),arclen_f(:,j),ydum,1,c1, &
                     c2,iflag,message)
...
!-------------------------------------------------------------------------------
f(:,:)=0
f(1,2:nr_f)=(xp_f(1,2:nr_f)+rin_f(2:nr_f)-rzmax_f(2:nr_f)-rzmin_f(2:nr_f))/2/a0
CALL SPLINE1_INTERP(k_vopt,nr_f,rinterp,f,nr_r,rho_r, &
                    value,iflag,message)
...
DO j=2,nr_f !Over flux surfaces
  ...
  ydum(1:mp_f(j))=btot_f(1:mp_f(j),j)/xnthinv_f(1:mp_f(j),j)/bp_f(1:mp_f(j),j)
  c1(1)=arclen_f(mp_f(j),j)
  CALL LINEAR1_INTEG(0,mp_f(j),arclen_f(:,j),ydum,1,c1, &
                     c2,iflag,message)
  ...
  avbp(j)=2*z_pi*c2(1)/c1(1)/dlobp_f(j)
...
DO k=1,3 !Over modes
  ...
  DO j=2,nr_f !Over flux surfaces
    ...
    ydum(1:mp_f(j))=SIN(k*ctheta_f(1:mp_f(j),j))*xndgradb_f(1:mp_f(j),j) &
                    /bp_f(1:mp_f(j),j)
    c1(1)=arclen_f(mp_f(j),j)
    CALL LINEAR1_INTEG(0,mp_f(j),arclen_f(:,j),ydum,1,c1, &
                       c2,iflag,message)
    ...
    fms=c2(1)/dlobp_f(j)
    ydum(1:mp_f(j))=ydum(1:mp_f(j))*grth_f(j)*ABS(btot_f(1:mp_f(j),j))
    CALL LINEAR1_INTEG(0,mp_f(j),arclen_f(:,j),ydum,1,c1, &
                       c2,iflag,message)
    ...
    fms=fms*c2(1)/dlobp_f(j)
    ydum(1:mp_f(j))=COS(k*ctheta_f(1:mp_f(j),j))*xndgradb_f(1:mp_f(j),j) &
                    /bp_f(1:mp_f(j),j)
    CALL LINEAR1_INTEG(0,mp_f(j),arclen_f(:,j),ydum,1,c1, &
                       c2,iflag,message)
...
DO j=2,nr_f !Over flux surfaces
  ...
  bmax=MAXVAL(ABS(btot_f(:,j)))

  !h=|B|/Bmax
  DO i=1,mp_f(j)

    hlin(i)=ABS(btot_f(i,j))/bmax
    hlin(i)=MIN(hlin(i),1.0_rspec)   

  ENDDO

  !<h**2>
  ydum(1:mp_f(j))=(hlin(1:mp_f(j))**2)/bp_f(1:mp_f(j),j)
  c1(1)=arclen_f(mp_f(j),j)
  CALL LINEAR1_INTEG(0,mp_f(j),arclen_f(:,j),ydum,1,c1, &
                     c2,iflag,message)
  ...
  !<h>   
  ydum(1:mp_f(j))=hlin(1:mp_f(j))/bp_f(1:mp_f(j),j)
  CALL LINEAR1_INTEG(0,mp_f(j),arclen_f(:,j),ydum,1,c1, &
                     c2,iflag,message)
  ...
  ydum(1:mp_f(j))=(1.0-(SQRT(1.0-hlin(1:mp_f(j)))*(1.0+hlin(1:mp_f(j))/2))) &
                  /bp_f(1:mp_f(j),j)/(hlin(1:mp_f(j))**2)
  CALL LINEAR1_INTEG(0,mp_f(j),arclen_f(:,j),ydum,1,c1, &
                     c2,iflag,message)
  ...
  hca=c2(1)/dlobp_f(j)
  ftl=1.0-h2a*hca
  ftu=1.0-(1.0-(1.0+ha/2)*SQRT(1.0-ha))*h2a/(ha*ha)
...
CALL FLUXAV_EVAL(x_f,nx_f,y_f,ny_f,cspln_f,rmag_f,zmag_f,1, &
                 pds,iflag,message)
...
30    IF(iflg == 0) THEN
      ...
      CALL FLUXAV_EVAL(x_f,nx_f,y_f,ny_f,cspln_f,x2,y2,1, &
                       pds,iflag,message)
      ...
      psi2=pds(1)
      IF((psivl-psi1)*(psivl-psi2) > 0.0) THEN
        ...
        IF(psi2-psi1 > 0.0) THEN    !psi is increasing
          ...
          DO WHILE(ABS(step) > serrt .AND. newti <= nitr8)
            ...
            CALL FLUXAV_EVAL(x_f,nx_f,y_f,ny_f,cspln_f,xn,yn,6, &
                             pds,iflag,message)
...
40    iflag=0
      CALL FLUXAV_EVAL(x_f,nx_f,y_f,ny_f,cspln_f,xn,yn,3, &
                       pds,iflag,message)
...
50    bp2=SQRT(pds(2)**2+pds(3)**2)/xn
    ...
    IF(itry.ne.0 .AND. &
       k_conv == 1) THEN
        ...
        psiin=psivl
        IF(ABS((psiout+psiin)/2-psivl) < 1.0e-6) GOTO 9999
...
REAL(KIND=rspec) :: &
  d,dy,h,hx,s0,sh,sp0,sph,u,v

REAL(KIND=rspec) :: &
  su(1:2),sv(1:2),sux(1:2),suy(1:2),svx(1:2),sxy(1:2)
...
!Internal functions
!-------------------------------------------------------------------------------
spln0(s0,sh,sp0,sph,h,d)=s0+d*(h*sp0+d*(3*(sh-s0)-(sph+2*sp0)*h &
                         +d*(2*(s0-sh)+(sph+sp0)*h)))
spln1(s0,sh,sp0,sph,h,d)=sp0+d*(6*(sh-s0)/h-2*(sph+2*sp0) &
                         +3*d*(2*(s0-sh)/h+(sph+sp0)))
spln2(s0,sh,sp0,sph,h,d)=6*(sh-s0)/h**2-2*(sph+2*sp0)/h+d*(2*(s0-sh)/h**2 &
                         +(sph+sp0)/h)*6
...
DO l=1,2
  ...
  i=2*(ly-1+l)
  j=i-1
  sv(l)=spln0(c(1,lxl,km1),c(1,lxl,kp1),c(1,lxl,k),c(1,lxl,kp2),dy,v)
  svx(l)=spln0(c(2,lxl,km1),c(2,lxl,kp1),c(2,lxl,k),c(2,lxl,kp2),dy,v)
...
INTEGER :: &
  i,ii,iold,j,kflarc,k_conv,mpmin
...
!Data on a contour
REAL(KIND=rspec) :: &
  bp0(mxncon),xp0(mxncon),yp0(mxncon)
...
!Minimum number of acceptable points on a surface
mpmin=50
...
CALL FLUXAV_EVAL(x_f,nx_f,y_f,ny_f,cspln_f,rmag_f,zmag_f,6, &
                 pds,iflag,message)
...
20    IF(j == nr_f) THEN
      ...
      IF(j == nr_f .AND. mp_f(j) < mpmin) THEN
        ...
        ELSE
          ...
          iflag=1
          message='FLUXAV_SURF(6)/ERROR:boundary reduction failed'
          GOTO 9999
      ...
      IF (cur0_f < 0.0) THEN
        ...
        bp_f(1:mp_f(j),j)=-bp_f(1:mp_f(j),j)
      ...
      !Copy arrays
      xp0(1:mp_f(j))=xp_f(1:mp_f(j),j)
      yp0(1:mp_f(j))=yp_f(1:mp_f(j),j)
      bp0(1:mp_f(j))=bp_f(1:mp_f(j),j)
      ...
      DO i=1,mp_f(j)-1
        ...
        iold=i+ii-1
        IF(iold > mp_f(j)) iold=iold-mp_f(j)+1
        xp_f(i,j)=xp0(iold)
        yp_f(i,j)=yp0(iold)
        bp_f(i,j)=bp0(iold)
      ...
      xp_f(mp_f(j),j)=xp_f(1,j)
      yp_f(mp_f(j),j)=yp_f(1,j)
      bp_f(mp_f(j),j)=bp_f(1,j)
      ...
      DO i=1,mp_f(j)-1
        ...
        arclen_f(i+1,j)=arclen_f(i,j)+SQRT((xp_f(i+1,j)-xp_f(i,j))**2 &
                        +(yp_f(i+1,j)-yp_f(i,j))**2)
...
DO j=nr_f,2,-1 !Over flux surfaces
  ...
  DO i=1,mp_f(j)
    ...
    CALL FLUXAV_EVAL(x_f,nx_f,y_f,ny_f,cspln_f,xp_f(i,j),yp_f(i,j),6, &
                     pds,iflag,message)
    ...
    !B field components
    capbr=-pds(3)/xp_f(i,j)
    capbz=pds(2)/xp_f(i,j)
    btot_f(i,j)=SQRT(capbr**2+capbz**2+(f_f(j)/xp_f(i,j))**2)

    !B field derivatives
    dbrdr=(pds(3)/xp_f(i,j)-pds(4))/xp_f(i,j)
    dbzdr=(-pds(2)/xp_f(i,j)+pds(5))/xp_f(i,j)
    dbtdr=(-f_f(j)/xp_f(i,j)+ffp_f(j)/f_f(j)*pds(2))/xp_f(i,j)
    dbdr=(capbr*dbrdr+capbz*dbzdr+f_f(j)/xp_f(i,j)*dbtdr)/btot_f(i,j)
    dbrdz=-pds(6)/xp_f(i,j)
    dbzdz=pds(4)/xp_f(i,j)
    dbtdz=ffp_f(j)/f_f(j)*pds(3)/xp_f(i,j)
    dbdz=(capbr*dbrdz+capbz*dbzdz+f_f(j)/xp_f(i,j)*dbtdz)/btot_f(i,j)

    !n dot grad(B)
    xndgradb_f(i,j)=-(capbr*dbdr+capbz*dbdz)/btot_f(i,j)
...
DO j=1,nr_f
  ...
  xnthinv_f(1:mp_f(j),j)=ABS(btot_f(1:mp_f(j),j))/bp_f(1:mp_f(j),j)

ENDDO

!-------------------------------------------------------------------------------
!Index of point nearest the plane of the axis on inside
!Maximum and minimum z on a contour
!-------------------------------------------------------------------------------
DO j=nr_f,2,-1 !Over flux surfaces
  ...
  DO i=1,mp_f(j)
    ...
    ydif=ABS(yp_f(i,j)-zmag_f)

    IF(xp_f(i,j) <= rmag_f .AND. ydif <= yref) THEN
...
DO j=2,nr_f
  ...
  bp0(1:mp_f(j))=1/bp_f(1:mp_f(j),j)
  c1(1)=arclen_f(mp_f(j),j)
  CALL LINEAR1_INTEG(0,mp_f(j),arclen_f(:,j),bp0,1,c1, &
                     c2,iflag,message)
...
DO j=2,nr_f
  ...
  c1(1)=arclen_f(mp_f(j),j)
  CALL LINEAR1_INTEG(0,mp_f(j),arclen_f(:,j),xnthinv_f(:,j),1,c1, &
                     c2,iflag,message)
...
DO j=2,nr_f
  ...
  CALL LINEAR1_INTEG(0,mp_f(j),arclen_f(:,j),xnthinv_f(:,j),mp_f(j), &
                     arclen_f(:,j), &
                     ctheta_f(:,j),iflag,message)
  ...
  ctheta_f(:,j)=ctheta_f(:,j)*grth_f(j)
...
DO j=2,nr_f
  ...
  bp0(1:mp_f(j))=1/bp_f(1:mp_f(j),j)/(xp_f(1:mp_f(j),j)**2)
  c1(1)=arclen_f(mp_f(j),j)
  CALL LINEAR1_INTEG(0,mp_f(j),arclen_f(:,j),bp0,1,c1, &
                     c2,iflag,message)
...
DO j=2,nr_f
  ...
  phit_f(j)=phit_f(j-1)+(f_f(j-1)*rm2_f(j-1)+f_f(j)*rm2_f(j)) &
